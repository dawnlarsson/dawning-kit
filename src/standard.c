...
/*
        ...
        src/compiler_memory.c. The allocator left for library.common.c. The
        rest stay concatenated in that exact order, because the order is load
        bearing and always was: errno precedes its consumers, and stream.c's
        last act is to undefine stdin, stdout and stderr and redefine them
        from descriptor numbers into the pointers a C program means -- so
        everything above that line gets the numbers and everything below gets
        the streams. Splitting them across files never expressed that; the
        include list did, and now the file order does.
...
#ifndef standard_input_descriptor
...
/*
        stream_is_terminal lives in library.common.c: line buffering and the
        applets ask the same question, and STREAM is skippable.
*/
