...
static bipolar bowl_bind_ro(string_address source, string_address target)
{
        /*
                ...
                Recursive bind would clone child mounts too, while the remount
                below does not reliably make every cloned child read-only.
                Do not import nested mounts into the fast view.
        */
                               ...
                               bipolar native_shell, bool isolated)
{
        ...
        /*
                ...
                pathname lets an absolute or climbing symlink inside the bowl
                resolve against the host root. Pin the object using the same
                in-root resolver setup uses, then execute that descriptor.
                O_PATH intentionally remains open across exec so shebang
                targets still work with execveat(AT_EMPTY_PATH).
        */
