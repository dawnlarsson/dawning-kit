/*
        Everything this shell does with a network.

        The commands -- ip, wget, ping -- and the whole stack underneath
        them, which used to be six files in a directory of its own that
        nothing else ever reached. They were one dependency tree, expanded
        in this order into this file: rtnetlink, DNS, then HTTP, which is
        reached only through TLS, which is reached only through the
        hashes and curves it needs, and DHCP last.

        Two things stay beside it and are not a split. src/net/wait.c is
        the one helper src/sh/tools.c also expands, and it is expanded
        there first, so it cannot live in here. src/net/anchors.inc is the
        trust anchors: a table, the way library.c's tables are tables.

        The sections below keep their own headings and their own include
        guards, so each still says what it is and what it refuses to know
        about the ones around it.
*/
