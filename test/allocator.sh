...
#
#           sh test/allocator.sh
#
#       test/allocator.c is built four times: once for each architecture
...
if ! gcc -O2 -std=gnu17 -w -DALLOCATOR_REFERENCE -o "$work/reference" \
        test/allocator.c 2>"$work/reference.log"
then
...
one()
{
        ...
        if ! $compiler $flags $extra $link -o "$work/$name" \
                test/allocator.c 2>"$work/$name.log"
        then
