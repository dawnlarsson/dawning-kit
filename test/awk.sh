...
compare 'trig full range' /dev/null 'BEGIN{split("-1e300 -1e100 -1e20 -1e10 -1 0 1 1e10 1e20 1e100 1e300",v);for(i=1;i<=11;i++)printf "%.12g %.12g\n",sin(v[i]+0),cos(v[i]+0)}'
compare 'atan2 exceptional quadrants' /dev/null 'BEGIN{split("-inf -1 -0 +0 1 +inf",v);for(i=1;i<=6;i++)for(j=1;j<=6;j++)printf "%.12g\n",atan2(v[i]*1,v[j]*1)}'
compare 'transcendental ranges' /dev/null 'BEGIN{for(i=-200;i<=200;i++){x=i/13;printf "%.10g %.10g %.10g %.10g\n",exp(x),log(exp(x)),sin(x),cos(x)}}'
compare 'power exceptional bases' /dev/null 'BEGIN{n=log(-1);print 1^n,n^0,(-1)^exp(1000),(-1)^(-exp(1000)),(-2)^4097,(-2)^4098}'
compare 'numeric conversion cached' /dev/null 'BEGIN{x="012";for(i=0;i<5;i++){print x+0,x==12,x=="012",x;x="013"}x="";print x+0,!x,x==0,x==""}'
...
compare 'printf both stars preserve order' /dev/null 'BEGIN{printf "[%*.*d]|%d\n", -7, 3, 7, 9}'
compare 'printf negative string precision' /dev/null 'BEGIN{printf "[%*.*s]|%d\n", 6, -1, "abcd", 9}'
compare 'printf counted nul around fields' /dev/null 'BEGIN{printf "a\0%*.*d\0z", 4, 2, 7}'
compare 'printf counted nul conversion' /dev/null 'BEGIN{printf "%\0s", 7}'
