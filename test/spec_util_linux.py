...
UTILITIES = (
    ...
    Utility("ipcs_live", modes=("bash",), stderr="loose", normalize=ul_norm_live,
            ...
            options=(*ul_flags("-m", "-q", "-s", "-a", "--human", "-b"),
                     Option("-i", ("@Q", "@M", "@S", "999999", "bad"), False)),
            # The detail, time, pid, creator and limits modes read fields this
            # census does not keep; each is asked for once rather than crossed.
            extra=(("-t",), ("-p",), ("-c",), ("-l",), ("-u",),
                   ("-m", "-t"), ("-q", "-p"), ("-s", "-c"), ("-m", "-l"),
                   ("-q", "-u"), ("-a", "-t", "--human"), ("-b", "-c", "-q"),
    ...
    Utility("lsipc_live", modes=("bash",), stderr="loose", normalize=ul_norm_live,
            ...
            options=(*ul_flags("--noheadings", "-b", "-c", "-J", "-n", "-l", "-P", "-r", "-t"),
                     Option("-o", ("KEY,ID", "KEY", "ID,OWNER,PERMS", "+CTIME", "BAD", "", "ID,CUID,CGID,UID,GID"), False)),
            # One System V resource is always selected: the global summary,
            # POSIX IPC, the export shapes and a custom time format are the
            # surface this build does not have, and stand on their own.
            operands=(("-m",), ("-q",), ("-s",)),
            extra=((), ("-g",), ("--global",), ("-M",), ("-Q",), ("-S",),
                   ("-N", "x"), ("-e", "-m"), ("-y", "-m"), ("--notruncate", "-m"),
                   ("--time-format=short", "-m"), ("--time-format=full", "-q"),
                   ("--time-format=iso", "-s"), ("--time-format=bad", "-m"),
                   ("-b",), ("-J",), ("-n",), ("-l",), ("-P",), ("-r",),
                   ("-m", "-i", "999999"), ("-m", "-i", "bad"), ("-l", "-i", "999999"),
                   ("-q", "-i", "@Q"), ("-m", "-i", "@M"), ("-s", "-i", "@S"),
                   ("-m", "-o", "ID,CUID,CGID,UID,GID"), ("-g", "-J"), ("-g", "-b")),
            stdin=("empty",), max_flags=4),
    Utility("ipcmk_live", modes=("bash",), stderr="loose", normalize=ul_norm_live,
            ...
            options=(Option("-M", ("4096", "0", "bad", "1"), False), Option("-S", ("3", "0", "4294967297", "2147483647", "bad"), False),
                     *ul_flags("-Q"), Option("-p", ("0640", "0600", "bad", "777"), False)),
            # -n names a POSIX object, and -q/-s/-m without it are the POSIX
            # spellings upstream refuses for the same reason.
            extra=(("-n", "name", "-Q"), ("-n", "name", "-s"), ("-n", "name", "-m", "4096"),
                   ("-n", "name", "-q"), ("-q",), ("-s",), ("-m", "4096"),
                   ("-n", "name", "-Q", "-p", "0640"), ("-n", "", "-Q"),
                   ("--name", "name", "--posix-queue")),
    ...
    Utility("ipcrm_live", modes=("bash",), stderr="loose", normalize=ul_norm_live,
            ...
            options=(Option("-q", ("@Q", "999999", "bad"), False), Option("-m", ("@M", "999999"), False),
                     ...
                     Option("-S", ("0",), False), *ul_flags("-v")),
            # Bulk removal and the POSIX names are the two shapes this build
            # refuses outright; every spelling of them is asked for once.
            extra=(("-a",), ("--all",), ("--all=shm",), ("--all=msg",), ("--all=sem",),
                   ("--all=bad",), ("-a", "shm"), ("-a", "-v"),
                   ("--posix-shmem", "x"), ("--posix-mqueue", "x"), ("--posix-semaphore", "x"),
                   ("-v", "-a", "-q", "999999")),
