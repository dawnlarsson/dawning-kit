...
if ! $host -O2 -w -DWORK="\"$work/reference\"" \
        -o "$work/t.reference" test/stream_reference.c 2> "$work/ref.err"; then
        printf '  the glibc reference did not build -- not run\n'
        sed 's/^/    /' "$work/ref.err" | head -10
        exit 2
fi

$host -O2 -w -o "$work/s.reference" test/stream_standard_reference.c \
        2>> "$work/ref.err" || exit 2
$host -O2 -w -o "$work/b.reference" test/stream_buffering_reference.c \
        2>> "$work/ref.err" || exit 2
...
for target in x86_64 arm64 riscv64; do
        ...
        if ! build_ours test/stream.c "$work/t.$target" "$target" "$cross" \
        ...
        if ! build_ours test/stream_standard.c "$work/s.$target" "$target" \
                "$cross" ""; then
                report no "$target standard build" "$(head -3 "$work/s.$target.err")"
                continue
        fi

        if ! build_ours test/stream_buffering.c "$work/b.$target" "$target" \
                "$cross" ""; then
