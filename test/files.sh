...
#
#           sh test/files.sh [directory holding our binaries]
#
...
group readlink
path_edge=$work/path-edge
mkdir "$path_edge"
: > "$path_edge/file"
ln -s 'file/..' "$path_edge/bad-walk"
ln -s absent "$path_edge/broken"
ln -s loop "$path_edge/loop"
...
answered 'resolve file separator' readlink -f "$fixture/alpha/"
answered 'resolve file dot' readlink -f "$fixture/alpha/."
answered 'resolve link to file separator' readlink -f "$fixture/sub/back/"
answered 'existing file parent' readlink -e "$fixture/alpha/.."
answered 'missing mode permits file separator' readlink -m "$fixture/alpha/"
answered 'missing mode permits link dot' readlink -m "$fixture/sub/back/."
answered 'resolve rejects invalid link target walk' readlink -f "$path_edge/bad-walk"
answered 'missing mode permits invalid link target walk' readlink -m "$path_edge/bad-walk"
answered 'missing mode permits link loop' readlink -m "$path_edge/loop"
...
answered 'file trailing separator' realpath "$fixture/alpha/"
answered 'file dot component' realpath "$fixture/alpha/."
answered 'file repeated separator' realpath "$fixture/alpha//"
answered 'file parent component' realpath "$fixture/alpha/.."
answered 'link to file separator' realpath "$fixture/sub/back/"
answered 'link to file dot' realpath "$fixture/sub/back/."
answered 'no symlinks still validates separator' realpath -s "$fixture/alpha/"
answered 'existing validates separator' realpath -e "$fixture/alpha/."
answered 'missing mode permits file separator' realpath -m "$fixture/alpha/"
answered 'no symlinks missing mode permits separator' realpath -s -m "$fixture/alpha/."
answered 'invalid link target walk' realpath "$path_edge/bad-walk"
answered 'no symlinks detects invalid link target' realpath -s "$path_edge/bad-walk"
answered 'missing mode permits invalid link target' realpath -m "$path_edge/bad-walk"
answered 'no symlinks permits missing referent' realpath -s "$path_edge/broken"
answered 'no symlinks existing requires referent' realpath -s -e "$path_edge/broken"
answered 'no symlinks rejects link loop' realpath -s "$path_edge/loop"
answered 'missing mode permits link loop' realpath -m "$path_edge/loop"
...
if python3 test/file_paths.py "$binaries"; then
        report ok
else
