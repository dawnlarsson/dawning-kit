...
#           sh test/spool.sh
#
#       Everything here is a comparison, for the reason test/stream.sh
...
if ! $host -O2 -w -DWORK="\"$work/reference\"" \
        -o "$work/reference.elf" test/spool_reference.c 2> "$work/ref.err"
then
...
for target in x86_64 arm64 riscv64; do
        ...
        if ! build_ours test/spool.c "$work/spool.$target" "$target" \
                "$cross" "$work/$target"
        then
